...
def mrmr_selection(
    ...
    with ThreadPoolExecutor(max_workers=n_jobs) as executor:
        futures = {executor.submit(calculate_mutual_information, X[i, :], y): i 
    ...
    for k in range(1, n_features):
        ...
        # Compute MRMR scores in parallel
        def compute_mrmr_score(idx):
            # Relevance: MI with target
            relevance = calculate_mutual_information(X[idx, :], y)
            
            # Redundancy: average MI with selected features
            redundancy = np.mean([
                calculate_mutual_information(X[idx, :], X[sel_idx, :])
        ...
        with ThreadPoolExecutor(max_workers=n_jobs) as executor:
            futures = {executor.submit(compute_mrmr_score, idx): idx 
                       for idx in remaining_features}
            mrmr_results = {}
            for future in as_completed(futures):
                idx = futures[future]
                mrmr_results[idx] = future.result()
        
        # Get scores in order
        mrmr_scores = [mrmr_results[idx] for idx in remaining_features]
